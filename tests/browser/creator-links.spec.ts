...
const creatorLinks = [
  { name: 'View pilldiff on GitHub', text: '', href: 'https://github.com/t3dotgg/pilldiff' },
...
test('keeps project and creator links visible in desktop and mobile headers', async ({ page }) => {
  await installAppHarness(page);
  await openCatalog(page);
  const header = page.getByRole('banner');
  const navigation = header.getByRole('navigation', { name: 'Project and creator links' });

  for (const width of [1440, 1250, 1200, 1100, 901, 900, 768, 701, 700, 600, 390, 320]) {
    await page.setViewportSize({ width, height: 900 });
    await expect(navigation).toBeVisible();
    await expect(navigation.getByRole('link')).toHaveCount(creatorLinks.length);
